...
private fun DrawScope.drawCaseAndDial(center: Offset, radius: Float) {
    // The clock floats the way a floating button does: one soft, subtle
    // pool of shade behind the case on every side, a touch heavier below,
    // never a crescent under the rim. Flat at 22/255 out to 0.90 of its
    // reach, where it is still hidden behind the case, then one soft ramp
    // to nothing at 1.105 r: on a short window the case nearly touches the
    // screen and a hard clip would show at the edge. The takes renderer
    // draws the same pool from the same numbers, so the app and the store
    // art stay one look.
    val shadeCenter = Offset(center.x, center.y + radius * 0.02f)
    val shadeRadius = radius * 1.085f
    drawCircle(
        brush = Brush.radialGradient(
            colorStops = arrayOf(
                0f to Color(0f, 0f, 0f, 22f / 255f),
                0.90f to Color(0f, 0f, 0f, 22f / 255f),
