...
Status: release 0.7 (versionCode 7), with the soft shade behind the case
that lifts the clock off the screen (docs/decisions.md D-013), was
submitted for Play review on 22 September 2026, and release 0.6
(versionCode 6) the same day. Release 0.5 (versionCode 5), with the
measured quartz tick and the fix for a tick a stalled frame could swallow
(docs/decisions.md D-012), was submitted on 19 September 2026. Release
0.4 (versionCode 4) was submitted on 17 September 2026; releases 0.3, 0.2
and 0.1 earlier the same day.
...
The tick is synthesized, never recorded: four variants of the same
movement, generated by `:tools:makeTick` and rotated once per second.
Each piece is a knock and a case that answers it, tuned by measuring
public domain recordings of real quartz clocks, which were used as a
measuring stick and never bundled. `:tools:tuneTick` is the tuning desk:
it prints the band balance, how much of the envelope is spent in the
first five milliseconds, how long the case rings, and how loud the case's
answer is, next to the same numbers for any recording left in
`build/tick-ref`. That measurement is what turned a tap on wood into a
knock on enamel, and it is why a stall in the frame loop can no longer
silence a step.
...
core/     its own Gradle module, pure Kotlin, zero Android imports: the
          ...
          hand picking, the spoken time, tick scheduling and rotation
...
./gradlew :tools:makeTick             # the quartet of quartz tick WAVs
./gradlew :tools:checkTick            # fails if the committed tick bytes drift
./gradlew :tools:tuneTick             # the tuning desk: measures the tick, and
                                      # any real-clocking recording in build/tick-ref
